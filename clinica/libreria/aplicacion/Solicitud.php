...
class Solicitud
{
	...
	private static $estados = array('Nuevo', 'Atendido','Rechazado');
	...
	public function agregar ($registro)
	{
		...
		if (isset($registro["registrar"]) and $registro["accion"]=="agregar") 
		{
			...
			if (isset($registro["adjunto"]))
			{
				...
				{
					$registro["mensajes"] = Pagina::getErrores("No se adjuntaron archivos.");
				}
			}
			else
			{
		...
		if (!Utils::isInteger($registro["IdUsuario"]))
		{
			$registro["errores"] = Pagina::getErrores( "El usuario es  incorrecto.");
		}
		
		if (!Utils::strIsValidate($registro["Asunto"]))
		{
			$registro["errores"] = Pagina::getErrores("El campo Asunto es obligatorio.");
		}
		
		if (!Utils::strIsValidate($registro["Contenido"]))
		{
			$registro["errores"] = Pagina::getErrores("El campo Contenido es obligatorio.");
		}
		
			
		if (!Departamento::esDepartamento($registro["IdDepartamento"]))
		{
			$registro["errores"] = Pagina::getErrores("El campo Departamento es incorrecto.");
		}
		...
		if(!$registro["errores"])
		{
				...
				$departamento = DAL::cleanSQLParam($registro["IdDepartamento"]);
				$nombreArchivo= DAL::cleanSQLParam($nombreArchivo);
				$sql = "INSERT INTO `solicitud` (IdSolicitud, Asunto, Contenido, Adjunto ,Adjunto_Nombre, Fecha, Estado, IdUsuario, IdDepartamento) VALUES ('', '$asunto', '$contenido','$adjunto' ,'$nombreArchivo', NOW(), 'Nuevo', '$usuario', '$departamento')";
				$result = DAL::executeNonQuery($sql);
				if ($result["Error"] != true)
				{
					$registro["mensajes"] = Pagina::getErrores("Se ingreso la solicitud exitosamente");
	...
	public function prepararFormularioAgregar( $registro)
	{
		...
		$ver= $this->consultar_tipo( $UsuarioRegistro->IdTipo);
	...
	public function validacionAgregar ( $registro)
	{
		...
		{
			unset($registro["adjunto"]);
		}
		
		
		if (!Utils::isInteger($registro["IdUsuario"]))
		{
			array_push($respuesta->errores, "El usuario es  incorrecto.");
		}
		...
		public function buscarSolicitudesUsuario($registro, $pagina = null,$IdUsuario)
	{
		if (isset($registro["buscar"]) || isset($registro["buscarEsp"]))
		{
			...
			if (isset($registro["solicitud"]))
			{
				...
				$sql = "SELECT 
						...
						WHERE S._rowid = '$id' AND S.IdUsuario = '$IdUsuario' LIMIT 1";
			}
			else
			{
				...
				$sql = "SELECT 
						...
						WHERE 1 $sqlEstado $sqlFecha $sqlDep AND S.IdUsuario = '$IdUsuario' ORDER BY S.IdSolicitud DESC";
	...
	public function buscarSU($registro, $pagina = null)
	{
		if (isset($registro["buscar"]) || isset($registro["buscarEsp"]))
		{
			$this->validacionBuscar($registro);
			...
			else
			{
				...
				{
					$IdDepartamento = $registro["IdDepartamento"];
					$sqlDep = "AND S.IdDepartamento = '$IdDepartamento'";
				}
				else
				{
					$sqlDep = "AND 1";
				...
				$sql = "SELECT 
						...
						LEFT JOIN `usuario` AS U ON S.IdUsuario = U.IdUsuario
						LEFT JOIN `departamento` AS D ON D.IdDepartamento = U.IdDepartamento
						WHERE 1 $sqlEstado $sqlFecha $sqlDep ORDER BY S.IdSolicitud DESC";
		...
		return $web->getFormBuscarSU($vars);
	}
	...
	public function buscar($registro, $pagina = null,$IdDepartamento)
	{
		if (isset($registro["buscar"]) || isset($registro["buscarEsp"]))
		{
			$this->validacionBuscar($registro);
			if (isset($registro["solicitud"]))
			{
				...
				$sql = "SELECT 
						...
						FROM `solicitud` AS S
						LEFT JOIN `usuario` AS U ON S.IdUsuario = U.IdUsuario
						WHERE S._rowid = '$id' AND S.IdDepartamento = '$IdDepartamento' LIMIT 1";
			}
			else
			{
				if (isset($registro["estado"]))
				...
				$sql = "SELECT 
						...
						LEFT JOIN `usuario` AS U ON S.IdUsuario = U.IdUsuario
						LEFT JOIN `departamento` AS D ON D.IdDepartamento = U.IdDepartamento
						WHERE 1 $sqlEstado $sqlFecha $sqlDep AND S.IdDepartamento = '$IdDepartamento' ORDER BY S.IdSolicitud DESC";
...
#####################################
	...
	public function getResultados($IdUsuario)
	{
		...
		if (!$this->accion)
		{
			$this->resultados = $this->obtenerTodos($IdUsuario,$_GET["pagina"]);
		}
	...
	public function getResultadosSU()
	{
		$web = new SolicitudWeb();
		...
		if (!$this->accion)
		{
			$this->resultados = $this->obtenerTodosSU($_GET["pagina"]);
		}
		
		
		 return $web->getFormResultadosSU($this->resultados);
	}
	...
	public function getResultadosAdministrador($IdDepartamento)
	{
		...
		if (!$this->accion)
		{
			$this->resultados = $this->obtenerTodosAdministrador($IdDepartamento,$_GET["pagina"]);
		}
		
		 return $web->getFormResultados($this->resultados);
	}
	
	public function obtenerTodosAdministrador ($IdDepartamento,$pagina=null)
	{
		$sql = "SELECT 
								S.*,
								CONCAT(U.Nombre,' ',U.Apellido) AS NombreUsuario
			...
			{
				$result = DAL::createDataSetPage($sql, 15, $pagina);
			}
	...
	public function getImprimirAdministrador($IdDepartamento)
	{
			$web = new SolicitudWeb();
		...
		if (!$this->accion)
		{
		 ...
		 return $web->getFormResultadosImprimir($this->resultados);
	}
	...
	public function obtenerTodos ($IdUsuario,$pagina=null)
	{
		$sql = "SELECT 
								S.*,
								CONCAT(U.Nombre,' ',U.Apellido) AS NombreUsuario
	...
	public function obtenerTodosSU($pagina=null)
	{
		$sql = "SELECT 
								S.*,
								CONCAT(U.Nombre,' ',U.Apellido) AS NombreUsuario
	...
	public function getResultadosNuevas($IdDepartamento)
	{
		$web = new SolicitudWeb();
		
		if (!$this->accion)
		{
			$this->resultados = $this->obtenerNuevas($IdDepartamento,$_GET["pagina"]);
		}
		
		return $web->getFormResultadosNuevas($this->resultados);
	}
	
	
	public function getResultadosNuevasSU($IdDepartamento)
	{
		...
		if (!$this->accion)
		{
			$this->resultados = $this->obtenerNuevasSU($IdDepartamento,$_GET["pagina"]);
		}
		
		
		return $web->getFormResultadosNuevasSU($this->resultados);
	}
	
	
	
	public function obtenerNuevas ($IdDepartamento,$pagina=null)
	{
		$sql = "SELECT 
								S.*,
								CONCAT(U.Nombre,' ',U.Apellido) AS NombreUsuario
	...
	public function obtenerNuevasSU($IdDepartamento,$pagina=null)
	{
		$sql = "SELECT 
								S.*,
								CONCAT(U.Nombre,' ',U.Apellido) AS NombreUsuario
	...
	public function contarNuevas ($IdDepartamento)
	{
		$sql = "SELECT 
						COUNT(*) AS Total
						FROM `solicitud` AS S
