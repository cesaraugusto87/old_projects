...
class PDF extends FPDF{
   ...
   function Header(){
	  ...
	  $this->Cell(70);
	  //Título
	  $this->Text(78,36,'Programacion Docente Programa Informatica');
...
$pdf->SetFont('Times','',7);
   ...
   $sql_oferta="select * from ofertacurso order by FechaIni,Secuencia Asc"; 
   ...
   if ($totalregistros > 0){ 
	  ...
	  $pdf->Cell(13,7,'Duracion',1,0,'C');
	  $pdf->Cell(13,7,'Turno',1,0,'C');
	  ...
	  do { 
		 ...
		 $pdf->Cell(13,7,cambiaf_a_normal($row_oferta_curso['FechaFin']),1,0,'C');
   	     $pdf->Cell(13,7,$row_oferta_curso['Duracion'],1,0,'C');
		 $pdf->Cell(13,7,$row_oferta_curso['Turno'],1,0,'C');
		 $pdf->Cell(12,7,$row_oferta_curso['Cupos'],1,0,'C');         
	     $pdf->Ln();  
	     $i=$i+1;
		 $totalhoras=$totalhoras+$row_oferta_curso['Duracion'];
         $cantPart=$cantPart+$row_oferta_curso['Cupos'];
		 $HorasPart=$HorasPart+($row_oferta_curso['Duracion']*$row_oferta_curso['Cupos']);
	  }while($row_oferta_curso = mysql_fetch_assoc($resultado_oferta));
      $pdf->Ln();
	  $pdf->Cell(70);
      $pdf->Cell(50,7,'Estadistica General',1,0,'C');
      $pdf->Ln();
	  $pdf->Cell(70);
      $pdf->Cell(35,7,'Total Cursos Programados',1,0,'C');
      $pdf->Cell(15,7,$i-1,1,0,'C');
      $pdf->Ln();
	  $pdf->Cell(70);
	  $pdf->Cell(35,7,'Total Horas Curso',1,0,'C');
      $pdf->Cell(15,7,$totalhoras,1,0,'C');
	  $pdf->Ln();
	  $pdf->Cell(70);
	  $pdf->Cell(35,7,'Cantidad Total Participantes',1,0,'C');
      $pdf->Cell(15,7,$cantPart,1,0,'C');
	  $pdf->Ln();
	  $pdf->Cell(70);
	  $pdf->Cell(35,7,'Horas - Participantes',1,0,'C');
      $pdf->Cell(15,7,$HorasPart,1,0,'C');
   }else{
