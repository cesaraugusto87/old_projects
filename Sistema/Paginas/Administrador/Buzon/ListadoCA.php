...
class PDF extends FPDF{
   ...
   function Header(){
	  ...
	  $this->Cell(70);
	  //Título
	  $this->Text(70,36,'Listado de Cartuchos En Centro Alterno.');
...
$pdf->SetFont('Times','',10);  
   ...
   $sql_oferta = "Select operador,reporte_secuencia,id_mod,idinicializacion,observaciones,id_ubicacion,numero_cartuchos,num_cartucho2,id_nomenclatura,fecha_ini,fecha_fin,fecha_exp,idnomenclatura,estante,id_frecuencia,cuerpo,tramo,boveda,gaveta, consecutivo from cartuchos, id_sistema where id_nomenclatura = idnomenclatura And id_ubicacion = 'CA' order by id_mod,fecha_ini,estante,cuerpo,tramo,boveda,gaveta"; 	
  ...
  if ($totalregistros > 0){ 
	  ...
	  do { 
		 ...
		 $pdf->Cell(45,5,$row_oferta_curso['observaciones'],1,0,'C');
 		 $pdf->Cell(12,5,$row_oferta_curso['tipo'],1,0,'C');
      ...
      $pdf->SetFont('Times','',10);         
      $pdf->Text(20,40,'* Actualmente No Existen Cartuchos en Centro Alterno Registrados en el Sistema....');
	  $pdf->SetFont('Times','',6);   
   }		 
$pdf->Output();   
?>
