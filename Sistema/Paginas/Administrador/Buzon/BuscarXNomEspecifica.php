<?php
   ...
   $sql="Select a.operador,a.reporte_secuencia,a.id_mod,a.idinicializacion,a.observaciones,a.id_ubicacion,a.numero_cartuchos,a.num_cartucho2,a.id_nomenclatura,a.fecha_ini,a.fecha_fin,a.fecha_exp,a.tipo,b.idnomenclatura,b.estante,b.id_frecuencia,b.cuerpo,b.tramo,b.boveda,b.gaveta from cartuchos As a, id_sistema As B where a.idinicializacion = upper('".$nomenclatura."') AND a.id_nomenclatura = b.idnomenclatura"; 		
...
function preguntar(idinicializacion,num,fecha,tipo,mod_car){
eliminar=confirm("¿Deseas eliminar este registro?");
if (eliminar)
	window.location.href = "FuncionesTodas.php?cartucho=" + idinicializacion + "&num=" + num + "&fecha=" + fecha + "&tipo=" + tipo + "&mod_car=" + mod_car +"&act=E";
else
				   ...
				   <div align="center">
				      <font color="#000000">
					     <a target="_self" href="javascript:preguntar('<?php echo $row_cursos['idinicializacion']; ?>','<?php echo $row_cursos['numero_cartuchos'] ?>','<?php echo $row_cursos['fecha_ini'] ?>','<?php echo $row_cursos['tipo'] ?>','<?php echo $row_cursos['id_mod'] ?>')">
						   <img src="../../../images/encontrado.gif" alt="Eliminar" width="15" height="12" border="0">						 </a>						 </a>				      </font>				   </div>
				</td>
             </tr>
          <?php }while ($row_cursos = pg_fetch_assoc($resultado)); ?>          
        </table>        
	  <?php }else{echo"<span class='Estilo12'>No existe el cartucho indicado en el Sistema</span>";}?>      </tr>
      <tr>
