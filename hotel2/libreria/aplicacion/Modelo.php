...
//import("libreria.aplicacion.ModeloWeb");
  ...
  $conexion_bd = pg_connect("dbname=Hotel  port=5432  user=postgres password=scaspc") ; 
...
#####################################################################
	public function AgregarCliente($registro)
	{	
		...
		$idcliente = DAL::cleanSQLParam($registro["idcliente"]);
		$nombre = DAL::cleanSQLParam($registro["nombre"]);
		$apellido = DAL::cleanSQLParam($registro["apellido"]);
		$domicilio = DAL::cleanSQLParam($registro["domicilio"]);
		$telefono = DAL::cleanSQLParam($registro["telefono"]);
	
	
		$sql = "INSERT INTO `cliente` (idcliente, nombre, apellido, domicilio, telefono) VALUES ('$idcliente', '$nombre', 
