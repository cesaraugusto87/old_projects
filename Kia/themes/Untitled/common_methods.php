...
if (strpos ($class, "blog_usernames_blog") !== FALSE) {
        if ($index > 0) {
          $output .= '&nbsp|&nbsp';
        }
        ob_start();?>
  ...
  if (!empty($var1) && !empty($var2) && !empty($var3)) {
    $output .= <<< EOT
      <table class="position" cellpadding="0" cellspacing="0" border="0">
        <tr valign="top">
          <td width="third-width">$var1</td>
          <td width="third-width">$var2</td>
          <td>$var3</td>
        </tr>
      </table>
EOT;
...
function art_get_content_cell_style($left, $right, $content) {
  ...
  return 'art-layout-cell content-wide';
}
...
ob_start();?>
  ...
  if ($author != '') {
ob_start();?>
    <img class="art-metadata-icon" src="<?php echo get_full_path_to_theme(); ?>/images/postauthoricon.png" width="14" height="14" alt="" /> <?php if ($output != '') {
      $output .= '&nbsp;|&nbsp;';
    }
    $output .= ob_get_clean();
    $output .= $author;
