...
function get_left_sidebar($left, $sidebar_left) {
  if (!empty($left)) return $left;
  if (!empty($sidebar_left)) return $sidebar_left;
  return NULL;
}

function get_right_sidebar($right, $sidebar_right) {
  if (!empty($right)) return $right;
  if (!empty($sidebar_right)) return $sidebar_right;
  return NULL;
}
 ...
 */
function Untitled_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb">'. implode(' | ', $breadcrumb) .'</div>';
  }
}

function Untitled_service_links_node_format($links) {
 ...
 * @ingroup themeable
 */
function Untitled_button($element) {
...
*/
function Untitled_img_assist_page($content, $attributes = NULL) {
